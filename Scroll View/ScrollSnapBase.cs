...
namespace UnityEngine.UI.Extensions
{
    public class ScrollSnapBase : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IScrollSnap, IPointerClickHandler
    {
        ...
        void Awake()
        {
            ...
            if (shopButton)
                shopButton.GetComponent<Button>().onClick.AddListener(() => { OnShopButton(); });
            if (homeButton)
                homeButton.GetComponent<Button>().onClick.AddListener(() => { OnHomeButton(); });
            if (boardButton)
                boardButton.GetComponent<Button>().onClick.AddListener(() => { OnBoardButton(); });
            if (friendsButton)
                friendsButton.GetComponent<Button>().onClick.AddListener(() => { OnFriendsButton(); });
